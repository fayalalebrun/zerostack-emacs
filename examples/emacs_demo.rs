...
"#
    ...
    fn write_demo_skills(repo: &Path) -> anyhow::Result<()> {
        ...
        write_demo_skill(
            ...
            "Use this skill when the user asks for a demo tour. Mention that the demo intentionally exercises read, list_dir, find_files, grep, write, edit, bash, and write_todo_list, with delayed reasoning so C-c C-c interruption can be tested.",
        )?;
        Ok(())
    }
...
- [x] built-in tool tour: read, list_dir, find_files, grep, write, edit, bash, write_todo_list
